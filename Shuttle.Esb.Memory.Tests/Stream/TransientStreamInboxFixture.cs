...
namespace Shuttle.Esb.Memory.Tests
{
    public class TransientStreamInboxFixture : InboxFixture
    {
        ...
        {
            TestInboxError(TransientStreamConfiguration.GetServiceCollection(), "transient-stream://./{0}", hasErrorQueue, isTransactionalEndpoint);
        }
        ...
        {
            TestInboxThroughput(TransientStreamConfiguration.GetServiceCollection(), "transient-stream://./{0}", 1000, count, 1, isTransactionalEndpoint);
        }
